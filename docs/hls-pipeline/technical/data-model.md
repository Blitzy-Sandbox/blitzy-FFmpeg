...
| `version` | `int` | The `hls_playlist_version` AVOption (default `-1`, meaning "auto-negotiate"). When `-1`, the muxer computes the minimum compliant `EXT-X-VERSION` from feature usage at `[libavformat/hlsenc.c:L1551-L1571]`. | `[libavformat/hlsenc.c:L253]` |
...
| `http_delete` | `AVIOContext *` | Persistent AVIOContext used to issue HTTP `DELETE` requests for `hls_delete_segments` (live sliding-window mode). Allocated lazily on first delete. | `[libavformat/hlsenc.c:L261]` |
