...
| `HLS_PROGRAM_DATE_TIME` set; `hls_max_nb_segments` aging removes oldest segment | `vs->initial_prog_date_time += en->duration` when oldest segment's PDT is not its own discontinuity anchor | Sliding window advances PDT seed forward | `[libavformat/hlsenc.c:L1124-L1125]` |
